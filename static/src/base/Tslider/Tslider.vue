...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() { // 初始化better-scroll
      ...
      // 屏幕改变大小slider宽度还是不变监听resize窗口改变事件
        if (!this.slider) {
          return;
        }
    ...
    methods: { // 将mounted方法封装进来
      // 横向滚动的一个宽度isResize标志位窗口事件
      _setSliderWidth(isResize) {
        this.children = (this.$refs.sliderGroup.children) / 10;
        // 获得列表的元素个数
      ...
      _inSlider() { // 初始化slide
        this.slider = new BScroll(this.$refs.slider, {
          scrollX: true,  // 横向滚动 X大写
          scrollY: false, // 纵向滚动  Y大写
          momentum: false, // 惯性
          snap: true,
          snapLoop: this.loop, // 循环等于传入的循环
          snapThreshold: 0.3,
          snapSpeed: 400
          // click: true如果加了这句 会与better-scroll
          // fastclick冲突造成轮播图点击补跳转页面 这句应该取消
        });
        // 维护currentPageIndex
